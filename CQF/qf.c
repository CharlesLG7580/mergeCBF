...
static __inline uint64_t incr(struct quotient_filter *qf, uint64_t idx)
{
	return (idx + 1) & qf->qf_index_mask;
}

static _inline uint64_t decr(struct quotient_filter *qf, uint64_t idx)
{
	return (idx - 1) & qf->qf_index_mask;
}

static _inline int is_occupied(uint64_t elt)
{
	return elt & 1;
}

static _inline uint64_t set_occupied(uint64_t elt)
{
	return elt | 1;
}

static _inline uint64_t clr_occupied(uint64_t elt)
{
	return elt & ~1;
}

static _inline int is_continuation(uint64_t elt)
{
	return elt & 2;
}

static _inline uint64_t set_continuation(uint64_t elt)
{
	return elt | 2;
}

static _inline uint64_t clr_continuation(uint64_t elt)
{
	return elt & ~2;
}

static _inline int is_shifted(uint64_t elt)
{
	return elt & 4;
}

static _inline uint64_t set_shifted(uint64_t elt)
{
	return elt | 4;
}

static _inline uint64_t clr_shifted(uint64_t elt)
{
	return elt & ~4;
}

static _inline uint64_t get_remainder(uint64_t elt)
{
	return elt >> 3;
}

static _inline bool is_empty_element(uint64_t elt)
{
	return (elt & 7) == 0;
}

static _inline bool is_cluster_start(uint64_t elt)
{
	return is_occupied(elt) && !is_continuation(elt) && !is_shifted(elt);
}

static _inline bool is_run_start(uint64_t elt)
{
	return !is_continuation(elt) && (is_occupied(elt) || is_shifted(elt));
}

static _inline uint64_t hash_to_quotient(struct quotient_filter *qf,
		uint64_t hash)
{
	return (hash >> qf->qf_rbits) & qf->qf_index_mask;
}

static _inline uint64_t hash_to_remainder(struct quotient_filter *qf,
		uint64_t hash)
{
